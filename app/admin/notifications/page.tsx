...
function toTitleCase(input: string) {
  return input
    ...
    .filter(Boolean)
    .map((word) => {
      // Preserve acronyms / words with numbers (e.g. CMSC, 128)
...
function getEntityHref(n: NotificationRow): string | null {
  ...
  switch (n.entity_type) {
    case "animal":
      if (n.event_type === 'animal.deleted') return '/admin/profiles';
      return `/admin/profiles/animal/${n.entity_id}`;
    case "animal_report":
      return `/admin/report/${n.entity_id}`;
    case "volunteer_call":
      return `/admin/volunteer/${n.entity_id}`;
    case "adoption_application":
      return `/admin/adoptions/${n.entity_id}`;
    default:
      return null;
  }
}
...
export default function AdminNotificationsPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      // Apply ordering according to admin selection
      if (sortBy === 'priority') {
